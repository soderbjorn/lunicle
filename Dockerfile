...
# The lunula toolkit resolves from Maven Central, so nothing toolkit-specific
# has to be copied in. There is no sibling checkout in the container, so the
# composite build in settings.gradle.kts never engages and the image builds
# against the version pinned in gradle/libs.versions.toml.
