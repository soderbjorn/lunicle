...
#   * the framing policy: see resolveAllowedFrameAncestors() in Application.kt.
#
...
require_dev_port_free() {
  ...
  if pgrep -f "$DEV_SERVER_MARKER" > /dev/null 2>&1; then
    ...
    echo "       Stop it with:  ./scripts/stop-all.sh" >&2
    ...
    echo "       Stop it with:  ./scripts/stop-all.sh          # keeps its data" >&2
...
# start_dev_server <frame-ancestors> — starts `:server:run` in the background and
...
start_dev_server() {
  ...
  for var in $LUNICLE_OAUTH_VARS; do
    value="${!var:-}"
    [[ -n "$value" ]] || continue
    # LUNICLE_GOOGLE_CLIENT_ID -> googleClientId, matching build.gradle.kts. The
    # LUNICLE_ prefix is stripped first: it namespaces the variable in a shared
    # deployment environment, but the -P property is already under `lunicle.`.
    prop="$(echo "${var#LUNICLE_}" | awk -F_ '{
      out = tolower($1)
  ...
  # same variable dev-db.sh reads to decide what to inspect. Forwarded ONLY
  ...
  # rule as -Pport.
  #
  # Until this existed, the variable was honoured by dev-db.sh and by nothing
