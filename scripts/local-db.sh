...
#
#   ./scripts/local-db.sh path              # where it is
#   ./scripts/local-db.sh inspect           # tables and row counts
#   ./scripts/local-db.sh dump [table]      # everything, or one table
#   ./scripts/local-db.sh sql "SELECT …"    # ad-hoc query
#   ./scripts/local-db.sh seed              # a project with issues to look at
...
cmd_seed() {
  ...
  if [[ "$(sqlite3 "$DB_PATH" "SELECT count(*) FROM projects WHERE name = 'Lunamux';")" != "0" ]]; then
    ...
    echo "(./scripts/local-db.sh wipe, then start the server, to begin again.)"
    exit 0
  fi
