...
#
#   ./scripts/run-container-standalone.sh              → http://localhost:8080/
#   ./scripts/run-container-standalone.sh --no-build   # skip the image rebuild
#   ./scripts/run-container-standalone.sh --no-open    # don't launch a browser
#
...
# tree, use ./scripts/run-dev-standalone.sh.
#
# There is no Issues tab and no site chrome here — this is the tracker alone.
# For the tracker inside the lunamux site:  ./scripts/run-container-embedded.sh
#
...
# ./scripts/container-down.sh (or ./scripts/stop.sh).
#
# Env:
#   LUNICLE_PORT   the tracker's port (default: 8080)
#
# See scripts/README.html for how this relates to the other three run scripts.
#
set -euo pipefail
...
# Production's framing policy. container-up.sh replaces any running container,
...
cat <<EOF
    ...
    ─────────────────────────────────────────────────────────────
      ...
      For the embed:  ./scripts/run-container-embedded.sh
