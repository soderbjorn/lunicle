...
fun Route.mcpApiRoutes(deps: McpDependencies) {
    ...
    post(ApiRoutes.MCP_ENABLED) {
        ...
        // Refused outright for a user an admin has not permitted, rather than
        // written and left inert. The gates read both flags, so a stored `true`
        // here would grant nothing — but it would be a row claiming the user has
        // agent access on, and the next admin to grant them permission would
