 ...
 *
 * Session id → impersonated user id. A session with no entry is being itself,
 * which is every session almost always.
 *
...
class Impersonations {
    private val bySession = ConcurrentHashMap<String, Long>()
    ...
    /** Who this session is acting as, or null when it is being itself — or has no session. */
    fun effectiveUserId(sessionId: String?): Long? = sessionId?.let(bySession::get)
...
data class Caller(
    ...
    val canImpersonate: Boolean get() = real?.isAdmin == true
}
