...
class OAuthClientStore(
    ...
    suspend fun find(clientId: String): OAuthClientRecord? = withContext(DatabaseDispatcher) {
    ...
    suspend fun touch(clientId: String): Unit = withContext(DatabaseDispatcher) {
     ...
     */
    suspend fun sweepStale(): Long = withContext(DatabaseDispatcher) {
...
class OAuthLoginStateStore(
    ...
    suspend fun find(id: String?): OAuthLoginStateRecord? = withContext(DatabaseDispatcher) {
    ...
    suspend fun delete(id: String): Unit = withContext(DatabaseDispatcher) {
...
class OAuthCodeStore(
     ...
     */
    suspend fun consume(code: String): OAuthCodeRecord? = withContext(DatabaseDispatcher) {
...
class OAuthTokenStore(
     ...
     */
    suspend fun validateAccessToken(token: String): OAuthTokenRecord? = withContext(DatabaseDispatcher) {
     ...
     */
    suspend fun rotateRefresh(refreshToken: String): RefreshResult = withContext(DatabaseDispatcher) {
        ...
        database.transactionWithResult {
            ...
            val owner = database.usersQueries.findById(row.user_id).executeAsOneOrNull()
                ?: return@transactionWithResult RefreshResult.Invalid
            if (owner.mcp_enabled == 0L) return@transactionWithResult RefreshResult.Refused
    ...
    suspend fun revokeFamily(familyId: String): Unit = withContext(DatabaseDispatcher) {
     ...
     */
    suspend fun revokeByToken(token: String): Unit = withContext(DatabaseDispatcher) {
        val hash = OAuthCrypto.sha256Hex(token)
        database.transaction {
     ...
     */
    suspend fun listGrants(userId: Long): List<OAuthGrant> = withContext(DatabaseDispatcher) {
     ...
     */
    suspend fun revokeForUserAndClient(userId: Long, clientId: String): Unit =
        withContext(DatabaseDispatcher) {
