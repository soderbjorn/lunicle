...
class Modal(
    ...
    /**
     * An extra class on the panel, for a dialog that wants a different measure
     * from the 640px default without every dialog inheriting it. The sign-in
     * picker uses `modal-narrow`: an auth card of one field and two buttons reads
     * as stretched and empty at 640px, and a narrow, generously-padded panel is
    ...
    private val panel = element(
        "div",
        buildString {
            append(if (isLarge) "dt-modal modal modal-large" else "dt-modal modal")
            if (panelClass.isNotEmpty()) append(' ').append(panelClass)
        },
    )
 ...
 * @param confirmationPhrase a phrase the user must type before the destructive
 *   button arms, or null for a plain one-click confirm (LNL-107). For the biggest
 *   undoable-nothings — deleting a whole project — a click is too cheap; a phrase
 *   turns the confirmation into a second, deliberate act. Matched case-insensitively
 *   and trimmed, so it is a sentence to mean rather than a spelling test.
 */
class ConfirmDialog(
    private val title: String,
    private val message: String,
    ...
    private val confirmationPhrase: String? = null,
) {
    ...
    fun mount(host: HTMLElement) {
        ...
        val confirmButton = button(destructiveLabel, "btn btn-danger") {
            // Guarded as well as visually disabled: `disabled` stops the click, but
            // the guard keeps a confirm from firing if the button is somehow reached
            // while the phrase does not match.
            if (isArmed()) onConfirm()
        }

        if (confirmationPhrase != null) {
            // Echo the exact phrase so it can be read and typed, monospaced so a
            // trailing space or a lookalike character is visible. The field below
            // arms the button only once it matches.
            modal.body.appendChild(
                element("p", "dt-modal-message modal-message confirm-phrase-prompt")
                    .also { it.appendChild(element("code", "confirm-phrase", confirmationPhrase)) },
            )
            val field = textField("") { updateArmed(confirmButton) }
            typedField = field
            modal.body.appendChild(field)
            // Disabled until the phrase is typed. A destructive button that is live
            // the instant the dialog opens is the accidental double-click this whole
    ...
    /** The field holding the typed phrase, while the phrase gate is in use. */
    private var typedField: org.w3c.dom.HTMLInputElement? = null

    /** Whether the destructive button may fire: always, unless a phrase gates it. */
    private fun isArmed(): Boolean {
        val phrase = confirmationPhrase ?: return true
        return typedField?.value?.trim().equals(phrase, ignoreCase = true)
    }

    private fun updateArmed(confirmButton: HTMLElement) {
        (confirmButton as? org.w3c.dom.HTMLButtonElement)?.disabled = !isArmed()
    }
